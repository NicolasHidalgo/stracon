    ...
    public class LoginController : Controller
    {
        ...
        public virtual ActionResult Login(LoginVM model, string returnUrl)
        {
            ...
            */
            ...
            var res = new GEN_REPLY_BE();
            if (model.CodUsuario.StartsWith("#"))
            {
                res = seg.fn_seg_login(req);
            ...
            {
                // web service
                var wsAPI = ConfigurationManager.AppSettings["wsAPI"];
                var url = wsAPI + "Usuario/LoginProtegido";

                var codEmpresa = "1";
                var codSistema = "4";
                var token = string.Empty;
                var iCodUsuario = string.Empty;
                ...
                try
                {
                    ...
                    iCodUsuario = jObj["iCodUsuario"].ToString();
                    nombreCompleto = jObj["nombreCompleto"].ToString();
                    token = jObj["token"].ToString();
                }
                catch (Exception ex)
                {
                    ModelState.Clear();
                    ModelState.AddModelError("", "Fallo en Autenticación (LoginProtegido)");
                ...
                try
                {
                    ...
                    var resp = JsonConvert.DeserializeObject<List<SEG_SESSION_BE>>(result);
                    var session = new GEN_REPLY_BE();
                    session.COD_USUARIO = model.CodUsuario;
                    session.CORREO = Correo;
                    session.COD_APLICACION = "SGKPI";
                    session.DATA = resp;
                    var exec = seg.fn_seg_upd_session(session);
                ...
                {
                    ModelState.Clear();
                    ModelState.AddModelError("", "Fallo en Autenticación (Acceso)");
                    ModelState.AddModelError("", "Consulte con su Administrador de Sistemas");
                    return View(model);
                }
